 ...
 */
   ...
   else 
   {
   
     {
       ...
       if (olg.sat ())
       {
         _visited.push_back (_input);
   		 aalta_formula::af_prt_set s;
   		 for (auto it = olg._evidence.begin (); it != olg._evidence.end (); ++ it) {
   		 	string str = aalta_formula::get_name (it->first);
   		 	aalta_formula* f = aalta_formula (str.c_str ()).unique ();;
   		 	if (!it->second)
 ...
 {  
  for (int i = 0; i < _scc_start; ++ i) {
    aalta_formula::af_prt_set edge;
    for (auto it = _visited_edges[i].begin (); it != _visited_edges[i].end (); ++ it) {
    	if ((*it) == aalta_formula::TRUE ()) {
    		edge.insert (*it);
    		continue;
    	}
    	aalta_formula *f = ((*it)->oper () > aalta_formula::Undefined) ? (*it) : (*it)->r_af ();
    	if (!f->is_until_marked ())
    		edge.insert (*it);
    }
  	print (edge);
  	//printf (", ");
  ...
  //print scc
  printf ("(\n");
  for (int i = _scc_start; i < _visited_edges.size (); ++ i) {
    aalta_formula::af_prt_set edge;
    for (auto it = _visited_edges[i].begin (); it != _visited_edges[i].end (); ++ it) {
    	if ((*it) == aalta_formula::TRUE ()) {
    		edge.insert (*it);
