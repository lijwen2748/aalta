 ...
 */
 ...
 nondeter_prog_state::get_next_pair (size_t pos)
 {
   ...
   */
   ...
   //aalta_formula::af_prt_set common = _formula->to_set ();
   if (common.empty ())           //at least one element of _unsatisfied_untils is satisfied 
   {
     ...
     aalta_formula* common_f = AND (common);
     if (is_invariant (common_f))       //common itself is an invariant
     {
        ...
        if (!check_until_never_satisfied (common_f)) {
            ...
            clear_f_node_map ();
            if (!P_common.empty ())
                return assignment_pair (P_common);
            else {
                update_avoid_with (common_f);
                return empty_pair ();
            }
        }
...
bool 
nondeter_prog_state::check_until_never_satisfied (aalta_formula* f) {
    ...
    for (auto it = P.begin (); it != P.end (); ++ it) {
        if ((*it)->oper () == aalta_formula::Until) {
            if (res == NULL)
                ...
                res = aalta_formula (aalta_formula::And, res, (*it)->mark_until ()).unique ();
        }
    }
    res = aalta_formula (aalta_formula::And, res, f->flatted ()).unique ();
    if (_global_flatted_formula != NULL)
        res = aalta_formula (aalta_formula::And, res, _global_flatted_formula).unique ();
    
    aalta_formula::af_prt_set P2 = res->SAT ();
    if (P2.empty ())
        return true;
    return false;
}
